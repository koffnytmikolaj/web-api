...
import TradeNoteTable from "./Table/TradeNoteTable";

function TradeNotes(props) {
  ...
  const [tradeNoteList, setTradeNoteList] = useState([]);
  ...
  const [tradeNotesLoaded, setTradeNotesLoaded] = useState(false);
  ...
  const appPath = '/trade_notes/';
  const [appPathExt, setAppPathExt] = useState(appPath);
  const fetchPath = process.env.REACT_APP_API + 'tradeNote/GetTradeNotes';
  const companyPath = process.env.REACT_APP_API + 'company/GetAllCompanies';
  const numberOfPagesPath = process.env.REACT_APP_API + 'tradeNote/GetNumberOfPages';
  ...
  function getTradeNotes() {

    let tradeNotesPath = fetchPath + getPageParams();
    (
      async () => {
        await fetch(tradeNotesPath, {credentials: "include"}).then(response => 
          response.json(),
            (error) => {
              if(error){
                setConnected(false);
            ...
            setTradeNoteList(data);
            setTradeNotesLoaded(true);
        });
      }
    )();
  }
  ...
  function renderAddTradeNoteButton() {
  ...
  function renderAddTradeNoteModal() {
  ...
  function renderAddTradeNote() {

    return (
      <>
        {renderAddTradeNoteButton()}
        {renderAddTradeNoteModal()}
      </>
    )
  }
  ...
  function renderContent() {
    ...
    let loading = (numberOfPagesLoaded + companiesLoaded + tradeNotesLoaded + 1) * 25;
